...
- **Web Search**: `kagi` (kagi-cli) instead of built-in web search when available. `kagi search "query"` and `kagi quick "question"` output JSON by default (pipe to `jq`); `kagi summarize --subscriber --url <url>` summarizes a page. Some features are also exposed as MCP tools (server: `kagi-mcp`).
